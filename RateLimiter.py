...
class IPStruct (object):
    ...
    def __init__ (self):
        ...
        # gbn: Try to disable captcha simply by whitelisting everyone. Didn't work...
        self.whitelisted = True     # if whitelisted ip (eg. Googlebot)
...
class RatelimiterSession (dict):
    ...
    def calc_block (self, rl, ua):
		...
		# gbn: trying to stop those captchas
                # self['blocked'] = 1
                self['blocked'] = 0
    ...
    def limit (self, **dummy_kwargs):
        ...
        with self.lock:
            ...
            if ipsession['blocked'] == 1:
                ...
                ipsession.log ('Redirected to captcha (but not really)')
                # gbn: 2017-01-29. Trying to stop captchas
		# cherrypy.response.headers['X-Ratelimiter-Denied'] = "503"
                ...
                ## gbn: 2017-02-04.  Why are we getting 403 for people who solve the captcha?
		#ipsession.log ('Blocked 403')
                ipsession.log ('Blocked 403 (but not really)')
